...
namespace Morpheus.Resource
{
    internal class FastEditorResourceLoader : ResourceLoaderBase
    {
        ...
        public override T LoadAsset<T>(string assetId)
        {
            ...
            if (assetInst == null)
            {
                Logger.LogError($"[FastEditorResourceLoader] Can't find AssetInst. AssetId: {assetId}");
                return null;
            }
        ...
        public override void UnloadAsset(string assetId)
        {
            ...
            if (assetInst == null)
            {
                Logger.LogError($"[FastEditorResourceLoader] AssetInst is in an error state. AssetId: {assetId}");
                return;
            }
