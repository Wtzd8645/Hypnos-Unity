...
namespace Morpheus.Editor
{
    ...
    public class SheetInfo : IEquatable<SheetInfo>
    {
        ...
        public bool IsFieldVaild()
        {
            ...
            {
                Logger.LogError($"[SheetInfo] Class name is not valid. Sheet: {sheetName}");
                return false;
            }

            if (fieldInfos.Count == 0)
            {
                Logger.LogError($"[SheetInfo] Field info is not valid. Sheet: {sheetName}");
                return false;
            }
        ...
        public bool IsValueVaild()
        {
            ...
            {
                Logger.LogError($"[SheetInfo] {sheetName} No data row to parse. LastRowNum: {LastRowIndex}");
                return false;
            }
            return true;
        }
