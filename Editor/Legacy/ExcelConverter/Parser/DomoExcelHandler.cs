...
namespace Morpheus.Editor
{
    public class DomoExcelHandler : IExcelHandler
    {
        ...
        public bool Parse(IWorkbook iWorkBook)
        {
            ...
            if (mainSheet == null)
            {
                Logger.LogError("[ExcelHandler] Can't find main sheet.");
                return false;
            }
            ...
            {
                Logger.LogError("[ExcelHandler] Column num not valid. Type: " + varTypeList.Count + " Name: " + varNameList.Count);
                return false;
            }

            if (rowCount > lastRowNumber)
            {
                Logger.LogError("[ExcelHandler] No data row to parse. LastRowNum: " + lastRowNumber);
                return false;
            }
        ...
        private void ParseTypeRow(IRow iRow)
        {
            ...
            for (int i = 1; i < columnNum; i++)
            {
                ...
                if (cell == null)
                {
                    Logger.Log("[ExcelHandler] Type row has " + (i - 1) + " column.");
                    return;
                }
                ...
                if (type == null)
                {
                    Logger.LogError("[ExcelHandler] Can't find type at column " + (i - 1));
                    return;
                }
        ...
        private void ParseNameRow(IRow iRow)
        {
            ...
            for (int i = 1; i < columnNum; i++)
            {
                ...
                if (cell == null)
                {
                    Logger.Log("[ExcelHandler] Name row has " + (i - 1) + " column.");
                    return;
                }
                ...
                {
                    Logger.LogError("[ExcelHandler] Name is duplicate at " + (i - 1) + " column.");
                    return;
                }
        ...
        public void GenerateXML(bool iIsEncrypt)
        {
            ...
            while (rowCount <= lastRowNumber)
            {
                ...
                {
                    Logger.LogError("[ExcelHandler] Verificate value failed. [" + rowCount + ", " + 0 + "]");
                    return;
                }
                ...
                for (int i = 0; i < columnNum; i++)
                {
                    ...
                    {
                        Logger.LogError("[ExcelHandler] Verificate value failed at [" + rowCount + ", " + i + "]");
                        return;
                    }
                }
        ...
        private void OutputXML(bool iIsEncrypt, XmlDocument iXMLDoc)
        {
            if (iIsEncrypt)
            {
                ...
                Logger.Log("[ExcelHandler] Generate encrypted XML successfully.");
            }
            else
            {
                iXMLDoc.Save(EditorKernel.Config.bytesOutputPath + "/" + mainSheet.SheetName + ".xml");
                Logger.Log("[ExcelHandler] Generate XML successfully.");
            }
        }
